...
def scenarios_list():
    ...
    return np.array([
        ...
        'city_9_sanfrancisco', 'city_10_austin', 'city_13_columbus', 
        'city_17_seattle',
        'city_1_losangeles', 
        'city_2_chicago', 
        'city_3_houston', 
...
def get_parameters(scenario):
    ...
    row_column_users = row_column_users_gen()
...
def label_gen(task, data, scenario, n_beams=64):
    ...
    if task == 'los':
        ...
        losChs = data['user']['LoS']
        losChs = np.where(losChs == -1, np.nan, losChs)
        plot_coverage(data['user']['location'], losChs, tx_pos=data['location'])
    elif task == 'bp':
        parameters, row_column_users, n_ant_bs, n_ant_ue, n_subcarriers = get_parameters(scenario)
...
def create_labels(data, task, scenario_name, n_beams=64):
    labels = label_gen(task, data, scenario_name, n_beams=n_beams)
